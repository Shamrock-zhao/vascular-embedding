...
def train(config_file, load_weights=False):
    ...
    if config['experiment']['data-loader'] == 'online':
        ...
        t_loader = data_loader(data_path, 'training', config['experiment']['sampling-strategy'], config['experiment']['image-preprocessing'], parse_boolean(config['training']['augmented']), 200000, int(config['architecture']['patch-size']), int(config['training']['max-zoom']))
        train_loader = data.DataLoader(t_loader, batch_size=int(config['training']['batch-size']), num_workers=4, shuffle=True)

        v_loader = data_loader(data_path, 'validation', config['experiment']['sampling-strategy'], config['experiment']['image-preprocessing'], False, 10000, int(config['architecture']['patch-size']), int(config['training']['max-zoom']))
        val_loader = data.DataLoader(v_loader, batch_size=256, num_workers=4, shuffle=True)

    elif config['experiment']['data-loader'] == 'online-multiple':
        
        dataset_names = config['folders']['dataset-names'].replace(' ', '').split(',')
        
        t_loader = data_loader(data_path, dataset_names, 'training', config['experiment']['sampling-strategy'], config['experiment']['image-preprocessing'], parse_boolean(config['training']['augmented']), 200000, int(config['architecture']['patch-size']), int(config['training']['max-zoom']))
        train_loader = data.DataLoader(t_loader, batch_size=int(config['training']['batch-size']), num_workers=4, shuffle=True)

        v_loader = data_loader(data_path, dataset_names, 'validation', config['experiment']['sampling-strategy'], config['experiment']['image-preprocessing'], False, 10000, int(config['architecture']['patch-size']), int(config['training']['max-zoom']))
        val_loader = data.DataLoader(v_loader, batch_size=256, num_workers=4, shuffle=True)
