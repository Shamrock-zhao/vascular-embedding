    ...
    def summary(self, set, iter):
        ...
        for key, value in logged.items():
            if logged.is_leaf(key):
                ...
                try:
                    out[key + '_mean'] = np.array([np.mean(col) for col in zip(*values)])
                    out[key + '_std'] = np.array([np.std(col) for col in zip(*values)])
                    out[key + '_mean_mean'] = np.mean(out[key + '_mean'])
